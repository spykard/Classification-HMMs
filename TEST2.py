...
trans = [["healthy"], ["healthy"]]  # ADD 1 MORE LABEL THAN SEQUENCES AND IT GIVES ERROR, ADD 1 LESS AND IT ENABLES SEMI-SUPERVISED TRAINING


hmm_leanfrominput_pos = HiddenMarkovModel.from_samples(NormalDistribution, 1, X=emis, labels=trans)
...
tt = MultivariateGaussianDistribution.from_samples(np.array([[1,2,3], [3,4,5], [15,18,20]]))
print(tt)
quit()

hmm_leanfrominput_neg = HiddenMarkovModel.from_samples(NormalDistribution, 1, X=emis, labels=trans)


print(hmm_leanfrominput_pos.log_probability([0.28]))
print(hmm_leanfrominput_neg.log_probability([0.15]))
quit()
...
distrs = [DiscreteDistribution({"just_initializing": 1.00}), DiscreteDistribution({"just_initializing": 1.00}), DiscreteDistribution({"just_initializing": 1.00}), DiscreteDistribution({"just_initializing": 1.00})]
#ICD = IndependentComponentsDistribution(distrs)
#x = ICD.from_samples(np.array(emis))
ICD = IndependentComponentsDistribution.from_samples([[1, 33, 3], [1, 2, 3]], distributions=distrs)
print(ICD)
quit()
...
# DOESN'T WORK WITH DISCRETE ANYWAY https://github.com/jmschrei/pomegranate/issues/402
distri_sum = [d1,d2,d3]
x = IndependentComponentsDistribution.from_samples([[11, 22, 33], [22, 22, 33]], distributions=distri_sum)  # Can only use One-Hot encoding as numbers
...
temp_array = np.array([[1, 0, 0], [1, 2, 1]])
xx = np.array(emis)
Dirch = DirichletDistribution.from_samples(xx[1, :])
x = [[1, 0, 0], [1, 2, 1]]
labels = [[1, 0, 0], [1, 2, 1]]
print(Dirch)
#hmm_leanfrominput = HiddenMarkovModel.from_samples(DirichletDistribution, 2, X=emis, labels=trans)
#print("READY", hmm_leanfrominput)
quit()
