...
from pomegranate import DiscreteDistribution, MultivariateGaussianDistribution
...
def HMM_NthOrder_Supervised(pos_data_train, neg_data_train, neu_data_train, pos_observations, neg_observations, neu_observations, data_test, labels_test, documentSentiments, targetnames, n_jobs, algorithm, graph_print_enable, silent_enable, silent_enable_2, n_order):
    ...
    pos_observations = [" ".join(document) for document in pos_observations.values]
    neg_observations = [" ".join(document) for document in neg_observations.values]
    neu_observations = [" ".join(document) for document in neu_observations.values]
    vectorizer = TfidfVectorizer()
    pos_tfidf = vectorizer.fit_transform(pos_observations)
    neg_tfidf = vectorizer.fit_transform(neg_observations)
    neu_tfidf = vectorizer.fit_transform(neu_observations)
    ...
    hmm_supervised_pos = HiddenMarkovModel.from_samples(MultivariateGaussianDistribution, n_components=9, X=pos_data, labels=pos_data, n_jobs=1, emission_pseudocount=0, verbose=True, state_names=['negneg','negneu','negpos','neuneg','neuneu','neupos','posneg','posneu','pospos'])
    print("NEXT HMM")
    # Build Neg Class HMM - !!! state_names should be in alphabetical order
    hmm_supervised_neg = HiddenMarkovModel.from_samples(MultivariateGaussianDistribution, n_components=9, X=neg_data, labels=neg_data, n_jobs=1, emission_pseudocount=0, verbose=True, state_names=['negneg','negneu','negpos','neuneg','neuneu','neupos','posneg','posneu','pospos'])
    hmm_supervised_neu = HiddenMarkovModel.from_samples(MultivariateGaussianDistribution, n_components=9, X=neu_data, labels=neu_data, n_jobs=1, emission_pseudocount=0, verbose=True, state_names=['negneg','negneu','negpos','neuneg','neuneu','neupos','posneg','posneu','pospos'])
...
for k, (train_indexes, test_indexes) in enumerate(k_fold.split(df_dataset.loc[:,"Sequences"], df_dataset.loc[:,"Labels"])):  # Split must be done before every classifier because generated object gets exhausted (destroyed)
    ...
    pos_observations = train_data.loc[train_data["Labels"] == "pos", ['Data']]
    neg_data_train = train_data.loc[train_data["Labels"] == "neg", ['Sequences']]
    neg_observations = train_data.loc[train_data["Labels"] == "neg", ['Data']]
    neu_data_train = train_data.loc[train_data["Labels"] == "neu", ['Sequences']]  
    neu_observations = train_data.loc[train_data["Labels"] == "neu", ['Data']]
    ...
    predicted_proba_2 = HMM_NthOrder_Supervised(pos_data_train, neg_data_train, neu_data_train, pos_observations, neg_observations, neu_observations, data_test, labels_test, documentSentiments, None, 1, set_algorithm, 0, 0, 0, 2)
