...
def Run_Preprocessing(dataset_name):
    ...
    pos_count_feature = [0 for i in range(2031)]  # 3 Extra columns, 3 Extra Features containing the counts (no sequential information)
    neg_count_feature = [0 for i in range(2031)]
    count = 0
    ...
    df = pd.DataFrame({'Labels': labels, 'Data': data, 'Neg_Feature': neg_count_feature, 'Pos_Feature': pos_count_feature})
...
for k, (train_indexes, test_indexes) in enumerate(k_fold.split(all_data, all_labels)):  # Split must be done before every classifier because generated object gets exhausted (destroyed)
    ...
    mode = "none"
    if mode == "Features":
        tfidf_dense = np.hstack((neg_feature_train, pos_feature_train))  # or column_stack 
    elif mode == "BoW_plus_Features":
        tfidf_dense = np.hstack((tfidf_dense, neg_feature_train, pos_feature_train))  # or column_stack  
    ...
    #clf = RandomForestClassifier(max_depth=None, max_features=300)
    #clf = ComplementNB() # 1
    ...
    if mode == "BoW_plus_Features":
        tfidf_dense_test = np.hstack((tfidf_dense_test, neg_feature_test, pos_feature_test))  # or column_stack  
    elif mode == "Features":
        tfidf_dense_test = np.hstack((neg_feature_test, pos_feature_test))  # or column_stack 
    print(tfidf_dense_test.shape)
