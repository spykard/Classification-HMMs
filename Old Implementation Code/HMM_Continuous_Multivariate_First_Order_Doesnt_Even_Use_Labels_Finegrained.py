...
def HMM_NthOrder_Supervised(data_train, data_test, labels_train, labels_test, totfidf_train, totfidf_test, documentSentiments, targetnames, n_jobs, algorithm, graph_print_enable, silent_enable, silent_enable_2, n_order):
    ...
    flatten_train = [item for sublist in totfidf_train.values for item in sublist]
    flatten_test = [item for sublist in totfidf_test.values for item in sublist] 
    tfidf_matrix_train = vectorizer.fit_transform(flatten_train).toarray()  # to dense
    tfidf_matrix_test = vectorizer.transform(flatten_test).toarray()  # to dense

    vocab = vectorizer.vocabulary_

    sentence_data_train = totfidf_train.values
    sentence_data_test = totfidf_test.values

    # Train
    current_ind = 0
    for i in range(instance_count):  # For each document

        tempp = []
        for sentence in sentence_data_train[i]:  # For each sentence
            #print(sentence_data_train[i][k])
            #tokenize_it = word_tokenize(data_train_new[i]) 
            tempp.append(tfidf_matrix_train[current_ind])
            #print(observations_train[current_ind])
            current_ind += 1

        observations_train.append(tempp)

    # Test
    # Train
    current_ind = 0
    for i in range(len(data_test)):  # For each document

        tempp = []
        for sentence in sentence_data_test[i]:  # For each sentence
            #print(sentence_data_train[i][k])
            #tokenize_it = word_tokenize(data_train_new[i]) 
            tempp.append(tfidf_matrix_test[current_ind])
            #print(observations_train[current_ind])
            current_ind += 1

        observations_test.append(tempp)

    p1 = 0.0
    p2 = 1.0
    distrs = []
    for i in range(len(vocab)):
        distrs.append(PoissonDistribution(p1,p2))

    # for i in range(len(observations_train)):
    #     print(len(observations_test[i]), len(data_test_transformed[i]))
    #     print(observations_test[i])
    #     print(data_test_transformed[i])
    #     quit()

    #hmm_leanfrominput_supervised = HiddenMarkovModel.from_samples(IndependentComponentsDistribution, 3, X=observations_train, labels=data_train_transformed, n_jobs=n_jobs, verbose=False, name="Finegrained HMM", independent_dists=distrs)
    hmm_leanfrominput_supervised = HiddenMarkovModel.from_samples(JointProbabilityTable, 3, X=observations_train, labels=data_train_transformed, n_jobs=n_jobs, verbose=False, name="Finegrained HMM")
    ...
    documentSentiments = ["neg", "neu", "pos"]
    ...
    if algorithm == 'viterbi':
        for x in range(0, len(data_test_transformed)):
            if len(data_test_transformed[x]) > 0:        
                try:        
                    #print(observations_test[x])
                    predict = hmm_leanfrominput_supervised.predict(observations_test[x], algorithm='viterbi')
                except ValueError as err:  # Prediction failed, predict randomly
                    print("Prediction Failed:", err)
                    predict = [random.randint(0, len(documentSentiments)-1)] 
        ...
        for x in range(0, len(data_test_transformed)):
            if len(data_test_transformed[x]) > 0:
                try:      
                    temp = hmm_leanfrominput_supervised.predict_log_proba(observations_test[x])
...
all_totfidf = df_dataset.loc[:,"Data"]
...
for k, (train_indexes, test_indexes) in enumerate(k_fold.split(all_data, all_labels)):  # Split must be done before every classifier because generated object gets exhausted (destroyed)
    ...
    totfidf_train = all_totfidf.reindex(train_indexes, copy=True, axis=0)
    data_test = all_data.reindex(test_indexes, copy=True, axis=0)
    labels_test = all_labels.reindex(test_indexes, copy=True, axis=0)
    totfidf_test = all_totfidf.reindex(test_indexes, copy=True, axis=0)
    ...
    set_algorithm = 'viterbi'
    predicted_proba_1 = HMM_NthOrder_Supervised(data_train, data_test, labels_train, labels_test, totfidf_train, totfidf_test, documentSentiments, None, 1, set_algorithm, 0, 1, 0, 1)
