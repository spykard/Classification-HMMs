...
class HMM_Framework:
    ...
    def __init__(self):
        ...
        self.trained_bow = None  # NEW 
        self.trained_vectorizer = None  # NEW 
        self.normalizer = None  # NEW
        self.text_data_train = None  # NEW   
        self.text_data_test = None  # NEW 
        self.length = 0
    ...
    def build(self, architecture, model, framework, k_fold, boosting=False,
        ...
        for train_index, test_index in cross_val.split(self.observations, self.golden_truth):
            ...
            self.text_data_train = self.text_data[train_index]
            self.text_data_test = self.text_data[test_index]

            self.train_bow_classifier(y_train, y_test)
    ...
    def _predict_pome_archit_b(self, state_test, obs_test, architecture_b_algorithm, formula_magic_smoothing):
        ...
        # Formula: score = π(state1) * ObservProb(o1|state1) * P(state2|state1) * ObservProb(o2|state2) * P(state3|state2) * ...  * P(staten|staten-1) * ObservProb(on|staten) , divided by the sequence length to normalize
            for k in range(predict_length): 
                ...
                current_text_data = self.text_data_test[k]
                ...
                if len(current_observations) > 0:  
                    for j in range(total_models):  # For every trained model, j is the index of current model across all containers
                        ...
                        try:
                            ...
                            current_obs_bow_prediction = self.predict_bow_classifier(current_text_data[0], j)  # ("Susy was a model", for_which_label_I_request_prediction)
                            current_temp_score *= current_obs_bow_prediction
                        except KeyError:
                        ...
                        for i in range(1, seq_length):  # This line is different between Pomegranate and HOHMM because the latter has a specific behavior for higher orders; it has multidimensional pi
                            ...
                            try:
                                ...
                                current_obs_bow_prediction = self.predict_bow_classifier(current_text_data[i], j)  # ("Susy was a model", for_which_label_I_request_prediction)
                                _obsprob_temp *= current_obs_bow_prediction
                            except KeyError:
                                # Out-of-vocabulary value
                                count_new_oov_local += 1
    ...
    def train_bow_classifier(self, labels_train, labels_test):
        for i in range(len(self.text_data_train)):  # Only do this on the Train data, the Test data remain as a sequence of sentences
            self.text_data_train[i] = ''.join(self.text_data_train[i])

        vectorizer = TfidfVectorizer(max_df=0.90, min_df=5, ngram_range=(1, 1), stop_words="english", use_idf=True, strip_accents='unicode')
        tfidf_matrix = vectorizer.fit_transform(self.text_data_train) 
        ...
        # Normalizer
        dummy_array = np.array([[0.0], [0.1], [1.0]])
        self.normalizer = MinMaxScaler((0.9, 1.1))  # Fit it once and keep using it later
        self.normalizer.fit(dummy_array)

        # Test that it works on itself (train)
        #tf_idf_matrix_test = vectorizer.transform(self.text_data_train)
        #predicted = clf.predict(tf_idf_matrix_test)
        #accuracy = metrics.accuracy_score(labels_train, predicted)
        #print(accuracy)
        #quit()
        
        self.trained_vectorizer = vectorizer
        self.trained_bow = clf

    def predict_bow_classifier(self, sentence, which_label_to_return):
        #print(self.hmm_to_label_mapping, self.trained_bow.classes_)
        tf_idf_matrix_test = self.trained_vectorizer.transform([sentence])
        predicted = self.trained_bow.predict_proba(tf_idf_matrix_test)  
        #print(predicted)  
        final = predicted[0][which_label_to_return]  # [[0.23639543 0.76360457]]
          
        return self.normalizer.transform(np.array(final).reshape(1,-1))[0][0]  
