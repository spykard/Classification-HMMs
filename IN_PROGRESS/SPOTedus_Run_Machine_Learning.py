...
def Run_Preprocessing(dataset_name):
    ...
    neg_count_feature = [0 for i in range(198)]  # 3 Extra columns, 3 Extra Features containing the counts (no sequential information)
    neu_count_feature = [0 for i in range(198)]
    pos_count_feature = [0 for i in range(198)]
    count = 0
    new_doc_flag = True
    with open('./Datasets/SPOT/data/spot-imdb-edus.txt', 'r') as file:
        for line in file:
    ...
    new_doc_flag = True
    with open('./Datasets/SPOT/data/spot-yelp13-edus.txt', 'r') as file:
        for line in file:
            if new_doc_flag == True:
                # binning [0-1], [2], [3-4]
...
for k, (train_indexes, test_indexes) in enumerate(k_fold.split(all_data, all_labels)):  # Split must be done before every classifier because generated object gets exhausted (destroyed)
    ...
    # Training    
    vectorizer = TfidfVectorizer(max_df=0.92, min_df=2, ngram_range=(1, 1), stop_words="english", use_idf=True, strip_accents='unicode')
