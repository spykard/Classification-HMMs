...
clustering_labeled_data = pickle.load(open('./Pickled Objects/Clustered_Labels_from_kMeans', 'rb'))
data_corresponding_to_labels = pickle.load(open('./Pickled Objects/Data_corresponding_to_Labels_from_clustering', 'rb'))
golden_truth = pickle.load(open('./Pickled Objects/Clustered_Labels_Golden_Truth', 'rb'))

clustering_labeled_data_test = pickle.load(open('./Pickled Objects/Clustered_Labels_from_kMeans_Test_Set', 'rb'))
...
artifically_labeled_data_merged = clustering_labeled_data + clustering_labeled_data_test
...
df = pd.DataFrame({'Labels': golden_truth_merged, 'Words': data_corresponding_to_labels_merged, 'Clustering_Labels': artifically_labeled_data_merged})

# 2. Remove empty instances from DataFrame, actually affects accuracy
emptySequences = df.loc[df.loc[:,'Clustering_Labels'].map(len) < 1].index.values
...
if True:
    ...
    hmm.build(architecture="B", model="Classic HMM", framework="hohmm", k_fold=5, boosting=False,                                                \
