...
class HMM_Framework:
    ...
    def _predict_pome_archit_b(self, state_test, obs_test, architecture_b_algorithm, formula_magic_smoothing):
        ...
        # !!! How to search with grep: egrep '^.*?\bword1\b.*?\bword2\b.*?$' *.neg
        # or even better               grep -Er 'word1.*word2'
            ...
            for k in range(predict_length): 
                ...
                if len(current_observations) > 0:  
                    for j in range(total_models):  # For every trained model, j is the index of current model across all containers
                        ...
                        ### NEW ###
                        if j == 0:
                            neg_score.append(current_temp_score)
                        else:
                            pos_score.append(current_temp_score)    
                    ...
                    ### NEW ###
                    for xxx in range(len(neg_score)):
                        if neg_score[xxx] > pos_score[xxx]:
                            print("0", end ="\t")
                        else:
                            print("1", end ="\t")
                    print()
