...
class HMM_Framework:
    ...
    def build(self, architecture, model, framework, k_fold, boosting=False,
        ...
        #DISABLED SINCE WE ARE USING text_instead_of_sequences NOW
        ...
        for train_index, test_index in cross_val.split(self.observations, self.golden_truth):
            ...
            if boosting == True:
                self.boosting_wrapper(text_instead_of_sequences, state_train, obs_train, y_train, state_test, obs_test, y_test, pome_algorithm, pome_verbose, pome_njobs, pome_smoothing_trans, pome_smoothing_obs, \
    ...
    def boosting_wrapper(self, text_instead_of_sequences, state_train, obs_train, y_train, state_test, obs_test, y_test, pome_algorithm, pome_verbose, pome_njobs, pome_smoothing_trans, pome_smoothing_obs, hohmm_high_order, hohmm_smoothing, hohmm_synthesize, pome_algorithm_t, architecture_b_algorithm, formula_magic_smoothing):
        """
        ...
        text_enable=False
        # Gets 70% accuracy with just 3% of the data, something is wrong (n_jobs=-1 was the issue)
        ...
        accuracies = []
        final_predict_on_train_or_test = "test"        
        ...
        # These are the train and test indices, single 90-10 split the remains the same the entire time
        temp_train_index = np.random.choice(np.arange(self.length), size=train_length, replace=False)
        train_index = np.zeros(self.length, dtype=bool)
        train_index[temp_train_index] = True

        # 0.
        weights_A = np.zeros(iterations)  # refers to the weight of each classifier on the Final Ensemble
        ...
        while t < iterations:
            ...
            # save or load doesn't matter, I do everything from scratch here
            #print(text_instead_of_sequences.head(3))
            #RKEA.generate_cluster_labels(text_instead_of_sequences, mode="spherical", n_components=700, cosine_sim_flag=False, cluster_count=random.randint(60,60))  # High Performance
            df = RKEA.load_from_files()
            #print(df.head(3))

            self.check_input_type(df.loc[:,"Clustering_Labels"], df.loc[:,"Words"], df.loc[:,"Labels"], text_instead_of_sequences, text_enable)
            self.verify_and_autodetect()
            ...
            state_train, obs_train, y_train = self.state_labels[train_index], self.observations[train_index], self.golden_truth[train_index]  # Needs to be ndarray<list>, not list<list>
            state_test, obs_test, y_test = self.state_labels[~train_index], self.observations[~train_index], self.golden_truth[~train_index]

            # 3-a. Bootstrap/Bagging with replacement
            #np.random.seed(seed=precomputed_seeds[t])    
            np.random.seed()
            random_multiplier = random.uniform(0.25, 0.45)     
            #random_sample_ind = np.random.choice(np.arange(train_length), size=int(train_length*random_multiplier), p=weights_D, replace=False)  # replace refers to something else here
                                                                                                                       # this matrix can also operate as a mapping
            random_sample_ind = np.random.choice(np.arange(train_length), size=int(train_length*random_multiplier), replace=False)  # replace refers to something else here
            ...
            #print(np.amax(weights_D))

            # 3-b. Training Phase
            self.set_unique_states_subset(state_train)
            self.set_unique_observations_subset(obs_train)
            self.train(random_sample_state, random_sample_obs, random_y, pome_algorithm, pome_verbose, pome_njobs, pome_smoothing_trans, pome_smoothing_obs, hohmm_high_order, hohmm_smoothing, hohmm_synthesize, weights_D)
            
            # 3-c. Prediction Phase on the TEST DATA 
            predict = self.predict(state_test, obs_test, pome_algorithm_t, architecture_b_algorithm, formula_magic_smoothing)
            self.result_metrics(y_test, predict, time_counter)
            #print(self.cross_val_metrics)
            #quit()
            ...
            print("err:", err, "out-of-vocabulary:", self.count_new_oov, "formula problems:", self.count_formula_problems)
            ...
            accuracies.append(self.cross_val_metrics['Accuracy'][1] * 100)
            accuracies.append(self.cross_val_metrics['Accuracy'][0] * 100)
            ...
            if err > 0.5:
                #continue
                cross_val_prediction_matrix_final.pop()
                mapping.pop()
                golden_truth.pop()
                break
        ...
        print(weights_A)
        print(accuracies)
        weights_A = weights_A[weights_A != 0]  # In case the loop ended in earlier than X iterations, remove the extra zeros at the end
        #quit()

        for i in range(len(cross_val_prediction_matrix_final)):
            if final_predict_on_train_or_test == "train":  # This axis is originally meant for cross validation folds but here it is used to store train_matrix on [1] and test_matrix on [0]
                cross_val_prediction_matrix_final[i].pop(0)  # The way it takes the value of train_matrix is quite weird with the use of a pointer and no actual assignment line [TODO FIX]
            elif final_predict_on_train_or_test == "test":
