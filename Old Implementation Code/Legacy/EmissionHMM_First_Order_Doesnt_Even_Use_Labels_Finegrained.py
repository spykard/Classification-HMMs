...
def HMM_NthOrder_Supervised(data_train, data_test, labels_train, labels_test, documentSentiments, targetnames, n_jobs, algorithm, graph_print_enable, silent_enable, silent_enable_2, n_order):
    ...
    print(data_train_transformed[0:10])    
    print(labels_supervised[0:10]) 
    hmm_leanfrominput_supervised = HiddenMarkovModel.from_samples(DiscreteDistribution, len(documentSentiments), X=data_train_transformed, labels=data_train_transformed, n_jobs=n_jobs, verbose=False, name="Finegrained HMM")
