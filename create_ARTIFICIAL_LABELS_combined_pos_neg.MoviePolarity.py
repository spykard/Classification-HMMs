...
def Run_Preprocessing(dataset_name):
    ...
    df_dataset = pd.DataFrame({'Data': data, 'Labels': labels})
...
def HMM_NthOrder_Supervised(data_train, data_test, labels_train, labels_test, documentSentiments, targetnames, n_jobs, algorithm, graph_print_enable, silent_enable, silent_enable_2, n_order):
    ...
    if pickle_load == 0:
        ...
        _, naive_bayes_data_test = np.split(data, [int(.8*len(data))])
        _, naive_bayes_labels_test = np.split(labels, [int(.8*len(labels))])
        naive_bayes_data_train, naive_bayes_labels_train = data, labels
        ...
        p1 = multiprocessing.Process(target=Create_Artificial_Labels_to_File, args=(data_train, labels_train, set(vocab), pipeline))
...
data_train = df_dataset.loc[:, "Data"]
labels_train = df_dataset.loc[:, "Labels"]
data_test = None
labels_test = None
